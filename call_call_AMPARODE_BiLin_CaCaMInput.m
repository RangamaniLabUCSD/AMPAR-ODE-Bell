%It calls Call_AMPARODE_bistable_CaCaMinput
close all
clear
clc
...
Influx= [0 1];
EnEx = [0 1];
resultsBi = zeros(length(Cparts),length(Pparts),length(Influx),length(EnEx),length(tspan),12); %C, P, inf, enex, species values, species 
resultsLin = zeros(length(Cparts),length(Pparts),length(Influx),length(EnEx),length(tspan),12); %C, P, inf, enex, species values, species 
styles = {"-",":","-."};
%bistable
%AMPAR bound
figure
...
for i = 1:length(Cparts)
    for j = 1:length(Pparts)
        for inf = 1:length(Influx)
            for enex = 1:length(EnEx)
                [t,y] = call_AMPARODE_bistable_CaCaMInput(Cparts(i), Pparts(j),Influx(inf),EnEx(enex), cacam); %CIC, PIC, EnEx, Influx
                resultsBi(i,j,inf,enex, :,:) = y;
...
for i = 1:length(Cparts)
    for j = 1:length(Pparts)
        for inf = 1:length(Influx)
            for enex = 1:length(EnEx)
                [t,y] = call_AMPARODE_linear_CaCaMInput(Cparts(i), Pparts(j),Influx(inf),EnEx(enex), cacam); %CIC, PIC, EnEx, Influx
                resultsLin(i,j,inf,enex, :,:) = y;
                plot(t,y(:,12)*(6.022e23*1e-12),styles{1})
                hold on
            end
        end
