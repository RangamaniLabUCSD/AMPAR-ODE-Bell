%It calls Call_AMPARODE_bistable/linear_CaCaMinput_scaleC1C2
close all
clear
clc
...
C1parts = [0:25:50];
C2parts = [0:25:50];
resultsBi = zeros(length(C1parts),length(C2parts),2,length(tspan),12); %C, P, cacam pulse, species values, species 
resultsLin = zeros(length(C1parts),length(C2parts),2,length(tspan),12); %C, P, cacam pulse, species values, species 
styles = {"-",":","-."};
%bistable
%AMPAR bound
figure
...
for k = 1:length(CICparts)
    for i = 1:length(C1parts)
        for j = 1:length(C2parts)
                    [t,y] = call_AMPARODE_bistable_CaCaMInput_scaleC1C2(CICparts(k), C1parts(i), C2parts(j), cacam); %CIC, PIC, EnEx, Influx
                    resultsBi(i,j,k, :,:) = y;
                    plot(t,y(:,12)*(6.022e23*1e-12),styles{1})
                    hold on
        end
    end
end
...
set(0,'defaultAxesFontSize', 28)
set(findall(gca, 'Type', 'Line'),'LineWidth',6)
pngfile = fullfile(pathname, 'ABound-varyC1C2-Bi.png');
saveas(gcf, pngfile);
...
for k = 1:length(CICparts)
    for i = 1:length(C1parts)
        for j = 1:length(C2parts)
                    [t,y] = call_AMPARODE_linear_CaCaMInput_scaleC1C2(CICparts(k),C1parts(i), C2parts(j), cacam); %CIC, PIC, EnEx, Influx
                    resultsLin(i,j,k, :,:) = y;
                    plot(t,y(:,12)*(6.022e23*1e-12),styles{1})
                    hold on
        end
...
set(0,'defaultAxesFontSize', 28)
set(findall(gca, 'Type', 'Line'),'LineWidth',6)
pngfile = fullfile(pathname, 'ABound-varyC1C2-lin.png');
saveas(gcf, pngfile);
